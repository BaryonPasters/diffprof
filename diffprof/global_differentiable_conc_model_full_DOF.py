...
from conc_pop_model_full_DOF import lgc_pop_vs_lgt_and_p50, get_param_grids_from_u_param_grids

@jjit
def get_predictions_from_singlemass_params_p50( ### NEED TO ADD THE SCATTER AT EACH P50
	singlemass_params_p50,
	tarr,
	p50_arr,
	u_be_grid,
	...
	avg_log_conc_p50 = jnp.sum(combined_u_weights.reshape((N_GRID, N_P50, 1))*lgc_p50_pop, axis=0)

	avg_log_conc_lgm0 = jnp.mean(avg_log_conc_p50,axis=0)

	avg_sq_lgconc_p50 = jnp.mean(jnp.sum(combined_u_weights.reshape((N_GRID, N_P50, 1))*((lgc_p50_pop)**2), axis=0),axis=0)
	sq_avg_lgconc_p50 = jnp.mean(jnp.sum(combined_u_weights.reshape((N_GRID, N_P50, 1))*(lgc_p50_pop), axis=0),axis=0)**2

	log_conc_std_lgm0 = jnp.sqrt(avg_sq_lgconc_p50-sq_avg_lgconc_p50)

	avg_sq_lgconc_multiple_p50 = jnp.sum(combined_u_weights.reshape((N_GRID, N_P50, 1))*((lgc_p50_pop)**2), axis=0)
	sq_avg_lgconc_multiple_p50 = jnp.sum(combined_u_weights.reshape((N_GRID, N_P50, 1))*(lgc_p50_pop), axis=0)**2

	log_conc_std_p50 = jnp.sqrt(jnp.abs(avg_sq_lgconc_multiple_p50-sq_avg_lgconc_multiple_p50))
...
def _loss(params, loss_data):
	...
	lgc_mean_targets_lgm0, lgc_std_targets_lgm0, lgc_mean_targets_lgm0_p50, lgc_std_targets_lgm0_p50 = targets
	mean_losses = 0
	std_losses = 0
	for ilgm in range(len(lgmasses)):
		...
		for ip50 in range(len(p50_arr)):
			mean_losses += _mse(lgc_mean_targets_lgm0_p50[ilgm][ip50],avg_log_conc_p50[ip50])
